/*
 * ============================================================
 *  멀티노드 재실 인원 노드  ★ Nicla Vision 판 (운용 v3)
 ...
 *   ③ 웜 리셋 후 카메라 안정화 지연 3초 (WDT 리셋은 GC2145를 리셋하지 못함)
 ...
 *   ※ 최종 안전망은 별도: 비전 노드 전원을 플러그 스케줄로 매일 재인가 (WORKLOG 09-01)
 * ============================================================
...
// --- v3 자가 복구 파라미터 ---
#define CAM_SETTLE_MS    3000              // 웜 리셋 후 GC2145 안정화 대기
...
static bool cameraInit() {                  // v3: 재시도 가능한 초기화
  for (uint8_t i = 0; i < CAM_TRIES; i++) {
    wd.kick();
    if (cam.begin(CAMERA_R160x120, CAMERA_RGB565, 30)) return true;
...
void setup() {
  ...
  Serial.println("\n[BOOT] Nicla occ node v3 (10s 단발 / 5분 버킷 / 워치독)");
  printResetReason();

  settle(CAM_SETTLE_MS);                    // ★v3 웜 리셋 시 GC2145 안정화 대기
