...
- **CLASS_02 환풍기**: 브로커 와일드카드 스캔으로 신규 플러그 `48f6eeb7ac6c` 식별 → `config/plugs.json` fan 매핑(9번째 장치, test_plugs 갱신). 실기동 테스트는 **CLASS_02 공청기·환풍기 플러그 둘 다 브로커 미접속(online:false)**이라 보류 — 교실 전원/WiFi 확인 후 재시도.
