...
작업 이력·세션 인계 기록. yyyy-mm 절, 최신이 위. 의미 있는 변경마다 갱신(커밋마다는 아님).
플랜: `docs/plan/CLAUDE_CODE_PLAN.md`, 드리프트: `docs/DRIFT.md`.
(구 `docs/PROGRESS.md` — 2026-09-12 전역 문서 구조 규칙에 따라 개명·월별 재편, 최신이 위.)

## 2026-09

### 환풍기 8대 재설치 완료 — 전 함대 실기동 검증 (2026-09-19)

수거·재설정 후 **16/16 전 플러그 브로커 접속**(C02 환풍기 MAC은 사용자 표의 80b54e2b05f0로 원복 확정 — 48f6eeb790d4는 별개 개체). 수동 모드 전환 → 환풍기 8대 일괄 ON: **7대 ~38 W 실가동 확인**(사용자 순회 확인), **C05만 릴레이 ON·0 W** — 환풍기 본체 스위치/결선 확인 필요. 전체 OFF·0 W 왕복 후 자동 모드 복귀. 이로써 공청기 8 + 환풍기 8 전 액추에이터가 대시보드·자동 제어 편입 완료(C05 본체만 미결).
...
### sudo 없는 서비스 재시작 경로 확립 + 임계·로스터 실반영 (2026-09-15)

원격제어 중 sudo 입력 불가 상황 해법: web·web_public·plugwatch 유닛은 `Restart=always`·`User=arduino`라 **ssh로 프로세스를 pkill하면 systemd가 자동 재기동** — sudo 불필요. 주의: pkill -f 패턴이 ssh 원격 명령줄 자신과 매치되면 셸만 죽는다 → `plugwatch[.]py` 식 브래킷 패턴 사용. 재시작으로 가동 임계 5 W(C03 6.9 W 가동 인정)·16장치 로스터·유령 노드 화면 차단 실반영 확인.

### 유령 노드 차단(분석·화면) + CLASS_02 환풍기 플러그 매핑 (2026-09-15)

- **점검 결과**: CLASS_01 환경노드는 수집 정상(24h 277행) — 화면 "제외"는 QC "CO₂ 유효 93.9% < 95%"(SCD30 일부 무효값). 실제 멈춘 노드는 **CLASS_06(node_E04537, 09-14 23:05 UTC~)**이고, 같은 시각 유령 `node_000500` 2행(id 148766·148774) 출현 — 재부팅 시 쓰레기 MAC(09-10 진단 유형). 전원 재인가 필요. **2행은 09-15 사용자 승인下 삭제 완료**(백업 출력 후 DELETE, 잔여 0).
- **미등록 노드 차단**: hub.py 수집 계층은 불변식+CI 가드로 유지하고, 정본 `nodes.json` 기준 허용목록을 ① analyst `prepare`/`fit_candidate`/occ 로더(판정·학습·상관 전부) ② webdata `env_nodes`/`stats`(전 화면)에 적용 — 유령 행은 저장돼도 판정·표시에서 배제(근본 차단은 펌웨어 v3.1 makeNodeId 재시도). weekly 테스트에 --nodes 누락 보완, 118 passed.
- **환풍기 플러그 8대 매핑**: 사용자 제공 표대로 `config/plugs.json` fan 슬롯 전부 기입(C01 48f6eeb7ac6c ~ C08 80b54e31a848, 총 16장치) — 최초에 브로커 스캔으로 잡힌 48f6eeb7ac6c를 CLASS_02로 오배정했다가 표 수신 후 정정(실제 CLASS_01 환풍기). 실기동 테스트는 접속되는 플러그부터 순차 — 매핑 시점 기준 fan 중 브로커 접속은 C01 1대뿐, CLASS_02는 공청기도 미접속(전원/WiFi 확인 필요).

### 제어 카드 ON/OFF 판정 칩 정리 — 플러그 실측 칩과 중복 제거 (2026-09-14, 사용자 결정)

제어·경보 카드에서 장치 ON/OFF 판정 칩과 플러그 실측 칩(전력·회전 애니메이션)이 중복 표기 — **실측 칩이 보이는 장치는 ON/OFF 칩 생략**, 부가 상태(유지·미평가)만 축약 칩으로 유지. 실측 칩이 없는 장치(플러그 미설치·오프라인·plugwatch 정지)는 종전 판정 칩 그대로(유일한 상태 정보). 판정↔실물 불일치는 실측 칩의 ⚠/cut 표시가 계속 담당. 가동 판별 임계 30→5 W(93df14e)는 코드 반영 완료 상태였고 **보드 웹 서비스 재시작만 미수행**이 "대기 6.9W" 오표시의 원인 — 재시작은 사용자 sudo.
...
### 수동 제어 실기동 검증 · 갱신/모션 버그 3종 · 반응성 개선 (2026-09-11 오후 ~ 09-12)
...
### 전력 UI 개편 + 수동/자동 제어 1단계 (2026-09-12, main 직접 커밋)
...
### 플러그 상태 파이프라인 — plugwatch + /api/plugs + 제어·경보 화면 (2026-09-11)
...
### Shelly Plug S Gen3 액추에이터 경로 실증 — 교무실 파일럿 (2026-09-11)
...
### 비전 v3 현장 진단 · 환경 유령 노드 교정 (2026-09-10, main 직접 커밋)
...
## 2026-06 ~ 2026-07 (프로젝트 초기 — 구 README 작업 로그)
