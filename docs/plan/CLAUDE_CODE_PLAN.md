...
- systemd `multinode_aq_web.service`(port 8501, 관리용) + `multinode_aq_web_public.service`(port 8502, `--public`: 내보내기·초기화 없음, Tailscale Funnel 공개용), `deploy.sh`가 `webapp.py|web/*|aq/*` 변경 시 재시작 대상에 포함.
