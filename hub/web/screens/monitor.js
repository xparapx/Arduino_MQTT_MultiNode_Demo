...
(() => {
  ...
  function renderStats(el) {
    ...
    if (d.daily) {
      ...
      if (d.dow) {
        const wpanel = (k, col) => `<div class="panel"><div class="tt" style="margin-bottom:6px;color:${cc(k, col)}">${TITLE[k]} 요일 × 시간 리듬 <span style="opacity:.7">(${d.days}일 중앙값 · 테두리 점멸 = 임계 초과)</span></div>${CH.dowheat(d.dow[k], thr[k], k, UNIT[k])}</div>`;
