/* 진단 & 추론 screens: dx-regime (B+H+G) / dx-band (C+D) / dx-action (E+F) /
   dx-scope (A, admin) / dx-history (I, admin). All sections read the one shared
   /api/analysis bundle via AQ.store; the section builders are the old page-2
...
(() => {
  ...
  const EMPTY = '<div class="panel"><div class="empty">아직 분석 결과가 없습니다 — analyst.py의 hourly / daily 타이머가 처음 실행되면 진단 화면이 나타납니다.<br><span class="mono">analyst.py run --mode hourly</span></div></div>';
  ...
  // ---- D: 전이확률 · 체류시간 --------------------------------------------------------
  function secD() {
    const cfg = A.cfg, tr = A.transition;
    let h = sec("transition", "blue", "전이확률 · 체류시간", `Δt = ${cfg.time.transition_dt_minutes} min ± ${cfg.time.transition_dt_tolerance} · 단절 쌍 ${tr ? num(tr.gap_pairs) : "—"}개 제외 · 체류시간은 하한`);
    if (tr) {
  ...
  function secE() {
    ...
    return sec("action", "red", "제어 판단 (하이브리드)", `hourly · 환풍기 ON CO₂&gt;${r.fan.on_co2} / OFF &lt;${r.fan.off_co2} · 공청기 ON VOC&gt;${r.purifier.on_voc} / OFF &lt;${r.purifier.off_voc} · 최소 ${r.min_run_minutes}분`)
      + `<div class="panel howto"><div><span class="sw jet"></span>배경 = 값 크기 (낮음 → 높음, jet)</div><div><span class="sw thr"></span>점선 = OFF 하한 · ON 상한 임계</div>`
...
;
  ...
  function renderInto(el, parts) {
    if (!A) return;
    if (A.empty) { el.innerHTML = EMPTY; return; }
