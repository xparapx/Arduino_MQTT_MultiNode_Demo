...
| 단계 | 내용 | 도구 |
|---|---|---|
| ① 측정 | 각 지점에서 공기질 11종 측정 | SEN55 + SCD30 (I2C) |
...
UNO R4 WiFi / Nano ESP32  →   로컬 mosquitto   →   hub.py ─→ SQLite ─→ dashboard.py
+ SEN55 + SCD30               또는                  (수집·저장)        (웹 :8501)
...
### MQTT 토픽 / 페이로드
- 발행 토픽: `multinode_sensor_demo/<node_id>/env`
- 구독: `multinode_sensor_demo/+/env` (`+` = 모든 노드)
- 페이로드(JSON, 11종): `{"node","t","pm1p0","pm2p5","pm4p0","pm10p0","sen_temp","sen_hum","voc","nox","co2","scd_temp","scd_hum","n"}` — `t`=측정시각(UTC), `n`=평균 샘플 수
- 온습도 대표값은 **SCD30**(`scd_temp`/`scd_hum`), SEN55 온습도는 비교용으로 함께 저장

---

## 하드웨어 구성

- **센서 노드**: Arduino **UNO R4 WiFi**(기본, 5V) 또는 **Nano ESP32**(대안, 3.3V) + Grove Base Shield + **Grove SEN55**(PM1.0/2.5/4/10·VOC·NOx·온습도) + **Grove SCD30**(CO₂·온습도) — 같은 I2C 버스 공유
...
2. 라이브러리 설치: **PubSubClient**(by Nick O'Leary), **Sensirion I2C SEN5x**(의존 라이브러리 Install All), **SparkFun SCD30 Arduino Library**
...
- **노드**: PubSubClient, Sensirion I2C SEN5x, SparkFun SCD30 Arduino Library, WiFiS3/WiFi(내장)
